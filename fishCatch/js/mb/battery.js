!(function(){
  ...
  // 炮台
    ...
    // 初始化
        ...
        if(nowDate - _self.$data.lastLaunchTime < 200) return
		...
		animationBeGold: function (viewPortProp, disFishList, $battert, nowXpoint, nowYpoint, _self) {
			...
			setTimeout(function () {
				...
				if (sumScore) {
					...
					_self.$el.$line.css('width', _self.$data.lineWidth + '%')

					if(_self.$data.lineWidth > 100 && _self.$data.moreSwitch) {
						_self.$data.moreSwitch = false
						_self.$data.lineWidth = 100
						_self.$el.$line.addClass('active').css('width', _self.$data.lineWidth + '%')
