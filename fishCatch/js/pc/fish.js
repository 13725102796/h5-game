!(function(){
  ...
  Fish.prototype = {
    ...
    renderFish: function (fishData, num) {
      var _self = this
      var screenWidth = this.$data.screen.h
      var screenHeight = this.$data.screen.w
      ...
      var iw = fishData.width													// 容器宽度
      var ih = fishData.height / Class								// 容器高度
      var stX // 初始left
      var stY
      var endY
      var endX																					// 目标left
      ...
      var transform = dir ? 'rotateZ(' + deg + 'deg)' : 'rotateZ(' + deg + 'deg) rotateY(180deg) scale(0.8)'
      ...
      this.$data.moveData = {
        ...
        transform: transform,
        progress: 0,
        progressUp: fishData.speed / 550000 * (1 + Math.random() * 0.5),
        type: 1 // type === 1 移动中
      }
      this.$el.$fish = $fish
    },
